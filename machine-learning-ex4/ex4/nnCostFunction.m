...
% Don't forget to get rid of the bias
Theta1_r = [ zeros(hidden_layer_size,1), Theta1(:,2:end) ];
Theta2_r = [ zeros(num_labels,1), Theta2(:,2:end) ];
T = Theta1_r(:)'*Theta1_r(:)+Theta2_r(:)'*Theta2_r(:);

J = -sum(sum(y_labels .* log(a3)+(1-y_labels) .* log(1-a3)))/m + lambda*(T)/(2*m);

% Back Propagation
for i=1:m
    delta3(i,:) = a3(i,:) - y_labels(i,:);
    delta2(i,:) = (delta3(i,:)*Theta2(:,2:end)) .* sigmoidGradient(z2(i,:));
